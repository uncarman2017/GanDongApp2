...
var BasePop = function(opts)
{
    ...
    this.$btnOk = $('<button class="btnItems btn-solid btn-lt">确定</button>');
    this.$btnCancel = $('<button class="btnItems btn-light btn-lt">取消</button>');
...
var MyCustomPop = function(opts)
{
    ...
    var $body = $("<div></div>");
    $body.append("<div class='title'><span class='icons " + icon + "'></span> " + opts.txtContent + "</div>");
...
//短信验证码确认框
var SmsPop = function(callback, isOkHide, text)
{
    if (!text) {
        text = "亲，请输入短信验证码";
    }
...
var MyAlert = function(msg, callback, type)
{
    ...
    var $body = $("<div></div>");
    $body.append("<div class='title'><span class='icons " + icon + "'></span> " + msg + "</div>");
