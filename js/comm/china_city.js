...
function RalativeCitys($parent_obj, $child_obj)
{
    var self = this;
    ...
    this.$obj_prov = $parent_obj.find("select");
    this.$obj_city = $child_obj.find("select");
    this.id_prov = this.$obj_prov.parent().attr('id');
    this.id_city = this.$obj_city.parent().attr('id');

    for(var i=0; i<_china_city.length; i++)
    {
        var l = _china_city[i].concat();
        var p = l.shift();
        this.provices.push(p);
        if (l.length == 0)
        {
            this.citys[p] = [p];
        }
        ...
        this.$obj_prov.append($("<option value='"+p+"'>"+p+"</option>"));
    }
    
    this.$obj_prov.off('change').on('change', function(){
        var id = $(this).parent().attr('id');
        var val = $(this).val();
        $(this).parent().find('.text').text(val);
        if(id == self.id_prov)
        {
            var citys = self.citys[val];
            self.$obj_city.empty();
            for(var i=0; i<citys.length; i++)
            {
                self.$obj_city.append($("<option value='"+citys[i]+"'>"+citys[i]+"</option>"));
            }
            self.$obj_city.parent().find('.text').text(citys[0]);
        }
    })
    
    this.$obj_city.off('change').on('change', function(){
        var val = $(this).val();
        $(this).parent().find('.text').text(val);
    });
    
    var val = $.trim(this.$obj_prov.parent().find('.text').text());
    if(val != "请选择省份")
    {
        var citys = self.citys[val];
        self.$obj_city.empty();
